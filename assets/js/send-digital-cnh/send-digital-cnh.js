...
let btnEnviarArquivo = document.getElementById("btn-enviar-arquivo");
let btnEnviarArquivoIcon = document.getElementById("btn-enviar-arquivo-icon");
let btnEnviarArquivoText = document.getElementById("btn-enviar-arquivo-text");
...
const initialEvents = (event) => {
  ...
  btnEnviarArquivoIcon.innerHTML = "outbox";
  btnEnviarArquivoText.innerHTML = "Enviar arquivo";
...
const fetchSnapCaptures = (snap) => {
  ...
  btnEnviarText.innerHTML = "Enviar foto";
};
...
const sendDigitalCNH = async () => {
  setTimeout(() => {
    ...
    btnEnviarArquivo.setAttribute("disabled", "");
    btnEnviarArquivoIcon.innerHTML = "cloud_upload";
    btnEnviarArquivoText.innerHTML = "Carregando...";
  });
  ...
  if (uploadedBase64File && uploadedBase64File.trim() !== "") {
    digitalCNH = uploadedBase64File.replace("data:image/jpeg;base64,", "");
  } else {
    digitalCNH = snapsCaptures[0].replace("data:image/jpeg;base64,", "");
  }
  ...
  if (file) {
    ...
    if (validTypes.includes(file.type)) {
      ...
      if (file.type.startsWith("image")) {
        fileIcon.src = "/assets/img/img-icon.png";
        fileIcon.alt = "Image File Icon";
      } else if (file.type === "application/pdf") {
        fileIcon.src = "/assets/img/pdf-icon.png";
        fileIcon.alt = "PDF File Icon";
      }
      fileName.textContent = file.name;

      convertToBase64(file);
    } else {
      alert("Por favor, selecione um arquivo no formato PDF, JPEG ou PNG");
    }
  }
}
...
function convertToBase64(file) {
  ...
  reader.onloadend = function () {
    uploadedBase64File = reader.result;
  };
